Mixture of Agents combines multiple LLMs to work together in cycles to answer a prompt. 

In each layer LLMs run in parallel and their outputs are then combined and fed into the next layer. The final layer's responses are given to the main LLM for the end response.

-- Why MoA/agentic workflows?

Accuracy⬆️: Allows LLMs to "think", refine, breakdown complex tasks into smaller sub-tasks.

Costs⬇️: Using a combination of smaller, open-source models can significantly reduce costs even if you are calling an LLM multiple times.
...
```
Resources: [skapadia3214-groq-moa](https://github.com/skapadia3214/groq-moa/blob/main/app.py)
